...
RUN_ID=${RUN_ID:-mindspeed_backend_$(date +%Y%m%d_%H%M%S)}
LOG_DIR=${LOG_DIR:-${REPO_ROOT}/logs/orin_ssm_${RUN_ID}}
OUTPUT_DIR=${OUTPUT_DIR:-${REPO_ROOT}/outputs/orin_ssm_${RUN_ID}}
SAVE_PATH=${SAVE_PATH:-${OUTPUT_DIR}/checkpoints}

BASE_CONFIG=${BASE_CONFIG:-/data/PonderLM/fcsong/LLaMA-Factory-adaptive/Llama_config/410m}
TOKENIZED_PATH=${TOKENIZED_PATH:-/data/PonderLM/uint16smallpile}
ORIN_TRAIN_SPLIT_NAME=${ORIN_TRAIN_SPLIT_NAME:-train}
ORIN_VALID_TOKENIZED_PATH=${ORIN_VALID_TOKENIZED_PATH:-${TOKENIZED_PATH}}
ORIN_VALID_SPLIT_NAME=${ORIN_VALID_SPLIT_NAME:-validation}
ORIN_PAD_TOKEN_ID=${ORIN_PAD_TOKEN_ID:-1}
...
SEED=${SEED:-42}
ORIN_SEED_MODE=${ORIN_SEED_MODE:-llamafactory}
ORIN_DATA_SEED=${ORIN_DATA_SEED:-${SEED}}
...
ORIN_DISABLE_SAVE=${ORIN_DISABLE_SAVE:-0}
...
ORIN_WANDB_LOG_STYLE=${ORIN_WANDB_LOG_STYLE:-llamafactory}
# Megatron emits the human-readable train line from one logging rank. The
# Orin loss_func now all-reduces lm loss across the data-parallel group before
...
export WANDB_RUN_ID
ORIN_BF16_AUTOCAST=${ORIN_BF16_AUTOCAST:-0}
...
export ORIN_MCORE_USE_NULL_ATTENTION_MASK=${ORIN_MCORE_USE_NULL_ATTENTION_MASK:-1}
export ORIN_CHUNKED_LM_LOSS_TOKENS=${ORIN_CHUNKED_LM_LOSS_TOKENS:-0}
export ORIN_LM_LOSS_UPCAST=${ORIN_LM_LOSS_UPCAST:-0}
export ORIN_DEBUG_LOSS_SHAPES=${ORIN_DEBUG_LOSS_SHAPES:-0}
...
LOOP_MAMBA_VARIANT=${LOOP_MAMBA_VARIANT:-orin_mamba2_fast}
LOOP_MAMBA_N_GROUPS=${LOOP_MAMBA_N_GROUPS:-8}
MAX_POSITION_EMBEDDINGS=${MAX_POSITION_EMBEDDINGS:-4096}

ORIN_MINDSPEED_ROOT=${ORIN_MINDSPEED_ROOT:-${REPO_ROOT}/third_party/orin_mindspeed/MindSpeed}
ORIN_MINDSPEED_LLM_ROOT=${ORIN_MINDSPEED_LLM_ROOT:-${REPO_ROOT}/third_party/orin_mindspeed/MindSpeed-LLM}

export NODE_IPS MASTER_ADDR MASTER_PORT NNODES NPROC_PER_NODE LOG_DIR OUTPUT_DIR SAVE_PATH
export TOKENIZED_PATH ORIN_TRAIN_SPLIT_NAME ORIN_VALID_TOKENIZED_PATH ORIN_VALID_SPLIT_NAME ORIN_PAD_TOKEN_ID
export ORIN_SEED_MODE ORIN_DATA_SEED
...
export ORIN_DISABLE_SAVE
if [ -z "${ORIN_MCORE_LOOP_LAYOUT+x}" ]; then
    if [ "${TENSOR_MODEL_PARALLEL_SIZE}" -gt 1 ] || [ "${SEQUENCE_PARALLEL}" = "1" ]; then
        ORIN_MCORE_LOOP_LAYOUT=sbh_tp
    else
        ORIN_MCORE_LOOP_LAYOUT=bsh
    fi
fi
export ORIN_MCORE_LOOP_LAYOUT
export ORIN_CHUNKED_LM_LOSS_CHECKPOINT=${ORIN_CHUNKED_LM_LOSS_CHECKPOINT:-0}
export TRANSFORMER_IMPL USE_FLASH_ATTN ATTENTION_BACKEND ORIN_MCORE_USE_NULL_ATTENTION_MASK
export ORIN_WANDB_LOG_STYLE
export TRAINING_BACKEND=${TRAINING_BACKEND:-mcore}
export PYTHONPATH="${ORIN_MINDSPEED_ROOT}:${ORIN_MINDSPEED_LLM_ROOT}:${REPO_ROOT}${PYTHONPATH:+:${PYTHONPATH}}"
...
if [ "${ORCHESTRATE}" = "1" ]; then
    ...
    if [ "${SKIP_BUSY_CHECK}" != "1" ]; then
        for host in "${nodes[@]}"; do
            busy_cmd="ps -eo pid,stat,cmd | grep -E 'torchrun|torch.distributed.run|llamafactory.launcher|deepspeed|pretrain_orin_ssm' | grep -v grep | grep -v '${RUN_ID}' | grep -v '${OUTPUT_DIR}' | head -20"
    ...
    for idx in "${!nodes[@]}"; do
        ...
        ssh -o BatchMode=yes -o ConnectTimeout=8 "${host}" \
            "RUN_ID='${RUN_ID}' REPO_ROOT='${REPO_ROOT}' NODE_IPS='${NODE_IPS}' MASTER_ADDR='${MASTER_ADDR}' MASTER_PORT='${MASTER_PORT}' NNODES='${NNODES}' NODE_RANK='${idx}' NPROC_PER_NODE='${NPROC_PER_NODE}' LOG_DIR='${LOG_DIR}' OUTPUT_DIR='${OUTPUT_DIR}' SAVE_PATH='${SAVE_PATH}' BASE_CONFIG='${BASE_CONFIG}' TOKENIZED_PATH='${TOKENIZED_PATH}' ORIN_TRAIN_SPLIT_NAME='${ORIN_TRAIN_SPLIT_NAME}' ORIN_VALID_TOKENIZED_PATH='${ORIN_VALID_TOKENIZED_PATH}' ORIN_VALID_SPLIT_NAME='${ORIN_VALID_SPLIT_NAME}' ORIN_PAD_TOKEN_ID='${ORIN_PAD_TOKEN_ID}' ORIN_SEED_MODE='${ORIN_SEED_MODE}' ORIN_DATA_SEED='${ORIN_DATA_SEED}' GLOBAL_BATCH_SIZE='${GLOBAL_BATCH_SIZE}' PER_DEVICE_TRAIN_BATCH_SIZE='${PER_DEVICE_TRAIN_BATCH_SIZE}' TENSOR_MODEL_PARALLEL_SIZE='${TENSOR_MODEL_PARALLEL_SIZE}' PIPELINE_MODEL_PARALLEL_SIZE='${PIPELINE_MODEL_PARALLEL_SIZE}' CONTEXT_PARALLEL_SIZE='${CONTEXT_PARALLEL_SIZE}' CONTEXT_PARALLEL_ALGO='${CONTEXT_PARALLEL_ALGO}' SEQUENCE_PARALLEL='${SEQUENCE_PARALLEL}' MAX_STEPS='${MAX_STEPS}' SEED='${SEED}' LR_SCHEDULER_STEPS='${LR_SCHEDULER_STEPS}' LEARNING_RATE='${LEARNING_RATE}' MIN_LR='${MIN_LR}' LOGGING_STEPS='${LOGGING_STEPS}' EVAL_INTERVAL='${EVAL_INTERVAL}' EVAL_ITERS='${EVAL_ITERS}' SAVE_INTERVAL='${SAVE_INTERVAL}' ORIN_DISABLE_SAVE='${ORIN_DISABLE_SAVE}' DATALOADER_NUM_WORKERS='${DATALOADER_NUM_WORKERS}' USE_DISTRIBUTED_OPTIMIZER='${USE_DISTRIBUTED_OPTIMIZER}' OVERLAP_GRAD_REDUCE='${OVERLAP_GRAD_REDUCE}' USE_FUSED_RMSNORM='${USE_FUSED_RMSNORM}' WANDB_PROJECT='${WANDB_PROJECT}' WANDB_EXP_NAME='${WANDB_EXP_NAME}' WANDB_RUN_ID='${WANDB_RUN_ID}' WANDB_SAVE_DIR='${WANDB_SAVE_DIR}' WANDB_GLOBAL_STEP_OFFSET='${WANDB_GLOBAL_STEP_OFFSET}' WANDB_API_KEY='${WANDB_API_KEY}' WANDB_BASE_URL='${WANDB_BASE_URL}' WANDB_ENTITY='${WANDB_ENTITY}' USE_MEGATRON_WANDB='${USE_MEGATRON_WANDB}' ORIN_WANDB_LOG_STYLE='${ORIN_WANDB_LOG_STYLE}' USE_MEGATRON_BF16='${USE_MEGATRON_BF16}' ORIN_MCORE_NATIVE='${ORIN_MCORE_NATIVE}' TRANSFORMER_IMPL='${TRANSFORMER_IMPL}' USE_FLASH_ATTN='${USE_FLASH_ATTN}' ATTENTION_BACKEND='${ATTENTION_BACKEND}' ORIN_MCORE_USE_NULL_ATTENTION_MASK='${ORIN_MCORE_USE_NULL_ATTENTION_MASK}' DISABLE_GMM_FP8='${DISABLE_GMM_FP8}' DISABLE_NAN_CHECK='${DISABLE_NAN_CHECK}' ORIN_BF16_AUTOCAST='${ORIN_BF16_AUTOCAST}' ATTN_IMPLEMENTATION='${ATTN_IMPLEMENTATION}' MORE_ITERATIONS='${MORE_ITERATIONS}' MEMORY_SIZE='${MEMORY_SIZE}' LOOP_MAMBA_VARIANT='${LOOP_MAMBA_VARIANT}' LOOP_MAMBA_N_GROUPS='${LOOP_MAMBA_N_GROUPS}' MAX_POSITION_EMBEDDINGS='${MAX_POSITION_EMBEDDINGS}' ORIN_MINDSPEED_ROOT='${ORIN_MINDSPEED_ROOT}' ORIN_MINDSPEED_LLM_ROOT='${ORIN_MINDSPEED_LLM_ROOT}' ORIN_MCORE_LOOP_LAYOUT='${ORIN_MCORE_LOOP_LAYOUT}' ORIN_CHUNKED_LM_LOSS_TOKENS='${ORIN_CHUNKED_LM_LOSS_TOKENS}' ORIN_CHUNKED_LM_LOSS_CHECKPOINT='${ORIN_CHUNKED_LM_LOSS_CHECKPOINT}' ORIN_LM_LOSS_UPCAST='${ORIN_LM_LOSS_UPCAST}' ORIN_DEBUG_LOSS_SHAPES='${ORIN_DEBUG_LOSS_SHAPES}' EXTRA_MEGATRON_ARGS='${EXTRA_MEGATRON_ARGS}' TRAINING_BACKEND='${TRAINING_BACKEND}' PYTHONPATH='${PYTHONPATH}' DRY_RUN='${DRY_RUN}' ORCHESTRATE=0 bash '${REPO_ROOT}/examples/orin_ssm/train_ssm9_mindspeed_backend_2node.sh'" \
...
path = os.environ["ORIN_VALID_TOKENIZED_PATH"]
split_name = os.environ["ORIN_VALID_SPLIT_NAME"]
...
train_cmd=(
    ...
    --rdzv_backend static
    "${REPO_ROOT}/pretrain_orin_ssm_mindspeed.py"
    --num-layers 24
    --hidden-size 1024
    ...
    --no-save-optim
    --no-save-rng
    --orin-config "${BASE_CONFIG}"
    --orin-tokenized-path "${TOKENIZED_PATH}"
    --orin-train-split-name "${ORIN_TRAIN_SPLIT_NAME}"
    --orin-valid-tokenized-path "${ORIN_VALID_TOKENIZED_PATH}"
    --orin-valid-split-name "${ORIN_VALID_SPLIT_NAME}"
    --orin-pad-token-id "${ORIN_PAD_TOKEN_ID}"
    --orin-seed-mode "${ORIN_SEED_MODE}"
    --orin-data-seed "${ORIN_DATA_SEED}"
    --orin-attn-implementation "${ATTN_IMPLEMENTATION}"
    --orin-more-iterations "${MORE_ITERATIONS}"
    --orin-memory-size "${MEMORY_SIZE}"
    --orin-loop-mamba-variant "${LOOP_MAMBA_VARIANT}"
    --orin-loop-mamba-n-groups "${LOOP_MAMBA_N_GROUPS}"
    --orin-max-position-embeddings "${MAX_POSITION_EMBEDDINGS}"
    --orin-output-dir "${OUTPUT_DIR}"
)

if [ "${ORIN_DISABLE_SAVE}" != "1" ]; then
...
if [ "${ORIN_BF16_AUTOCAST}" = "1" ]; then
    train_cmd+=(--orin-bf16-autocast)
else
    train_cmd+=(--no-orin-bf16-autocast)
fi
train_cmd+=(--orin-mcore-native)
...

{
    ...
    echo "SEED=${SEED}"
    echo "ORIN_PAD_TOKEN_ID=${ORIN_PAD_TOKEN_ID}"
    echo "ORIN_SEED_MODE=${ORIN_SEED_MODE}"
    echo "ORIN_DATA_SEED=${ORIN_DATA_SEED}"
    ...
    echo "ORIN_TRAIN_SPLIT_NAME=${ORIN_TRAIN_SPLIT_NAME}"
    echo "ORIN_VALID_TOKENIZED_PATH=${ORIN_VALID_TOKENIZED_PATH}"
    echo "ORIN_VALID_SPLIT_NAME=${ORIN_VALID_SPLIT_NAME}"
    ...
    echo "ORIN_WANDB_LOG_STYLE=${ORIN_WANDB_LOG_STYLE}"
    echo "USE_MEGATRON_BF16=${USE_MEGATRON_BF16}"
    echo "ORIN_MCORE_NATIVE=${ORIN_MCORE_NATIVE}"
    echo "ORIN_MCORE_LOOP_LAYOUT=${ORIN_MCORE_LOOP_LAYOUT}"
    echo "TRANSFORMER_IMPL=${TRANSFORMER_IMPL}"
    echo "USE_FLASH_ATTN=${USE_FLASH_ATTN}"
    echo "ATTENTION_BACKEND=${ATTENTION_BACKEND}"
    echo "ORIN_MCORE_USE_NULL_ATTENTION_MASK=${ORIN_MCORE_USE_NULL_ATTENTION_MASK}"
    echo "ORIN_CHUNKED_LM_LOSS_TOKENS=${ORIN_CHUNKED_LM_LOSS_TOKENS}"
    echo "ORIN_CHUNKED_LM_LOSS_CHECKPOINT=${ORIN_CHUNKED_LM_LOSS_CHECKPOINT}"
    echo "ORIN_LM_LOSS_UPCAST=${ORIN_LM_LOSS_UPCAST}"
    echo "EXTRA_MEGATRON_ARGS=${EXTRA_MEGATRON_ARGS}"
    echo "DISABLE_GMM_FP8=${DISABLE_GMM_FP8}"
    echo "DISABLE_NAN_CHECK=${DISABLE_NAN_CHECK}"
    echo "ORIN_BF16_AUTOCAST=${ORIN_BF16_AUTOCAST}"
